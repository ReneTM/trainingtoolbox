...
bluntMeleeData <-
[
	//{ model = "models/weapons/melee/v_riotshield.mdl", itemName = "riotshield" , alias = "riotshield"} // We change the name so we dont collide with any mods
	{ model = "models/weapons/melee/v_riotshield.mdl", itemName = "riot_shield" , alias = "riotshield"}
...
function GiveMelee(player, melee){
	...
	if(sharps.find(melee) != null || blunts.find(melee) != null){
		...
		ClientPrint(player, 5, "The " + melee + " is not available on this campaign.")
		ClientPrint(player, 5, "Take one of these: " + meleeAliases)
	}
}
